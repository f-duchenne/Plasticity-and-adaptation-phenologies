...
resf$Estimate[grepl("temp_",resf$varia) | resf$varia %in% c("Annee2","Altitude")]/
resf$sd.x[grepl("temp_",resf$varia) | resf$varia %in% c("Annee2","Altitude")]
...
contrib_tab=subset(resf,type %in% c("plast","adapt")) %>% dplyr::group_by(Speciesgen,ORDRE,FAMILLE,Species,mfd) %>% 
dplyr::summarise(contrib_plast=sum(contrib[type=="plast"],na.rm=T),contrib_plast_lwr=sum(contrib_lwr[type=="plast"],na.rm=T),
contrib_plast_upr=sum(contrib_upr[type=="plast"],na.rm=T),
contrib_adapt=sum(contrib[type=="adapt"],na.rm=T),
contrib_adapt_lwr=sum(contrib_lwr[type=="adapt"],na.rm=T),contrib_adapt_upr=sum(contrib_upr[type=="adapt"],na.rm=T))
...
stat_cor(p.accuracy = 0.01, r.accuracy = 0.01,label.x=0.4,label.y=0.32)
...
pl1=ggplot()+
geom_jitter(data=bidon_unscale,aes(x=temp_75_165,y=Jour.de.collecte,color=Annee),height=0,width=0.05)+
...
geom_point(data=newdat,size=3,color="black",stroke = 2,aes(x=x,y=predicted,color=group2,fill=group2,shape=scenario))+
...
pl2=ggplot()+
geom_jitter(data=bidon_unscale,aes(x=temp_30_120,y=Jour.de.collecte,color=Annee),height=0,width=0.05)+
scale_color_gradient(low=brewer.pal(8, "Blues")[c(2)],high=brewer.pal(9, "Blues")[9],name="Year of observation",breaks=c(1960,1980,2000))+
new_scale_colour()+
geom_ribbon(data=b,aes(x=x,y=predicted,color=group2,fill=group2,ymin=conf.low,ymax=conf.high),alpha=0.2,col=NA)+
geom_line(data=b,aes(x=x,y=predicted,color=group2,fill=group2),size=1)+
...
pl3=ggplot()+
geom_jitter(data=bidon_unscale,aes(x=temp_30_120,y=Jour.de.collecte,color=Annee),height=0,width=0.02)+
...
scale_shape_manual(values=c(21,24),name="Estimated position\non reaction norm")+labs(shape="")+
 guides(fill = guide_legend(order = 3,override.aes = list(linetype = 1)),color = guide_legend(order = 3,override.aes = list(linetype = 1,shape=NA)),
 shape=guide_legend(order=4,override.aes = list(size=1)))+theme(legend.key.size = unit(0.3, 'cm'))
...
geom_pointrange(aes(ymin=lwr,ymax=upr))+coord_flip()+geom_hline(yintercept=0,linetype="dashed")+ggtitle("")+theme_bw()+
theme(strip.background = element_blank(),panel.border=element_blank(),panel.grid=element_blank(),axis.line=element_line(),
...
pl1=ggplot(data=subset(resf,varia=="Annee2"),aes(x=mean,y=Estimate))+
